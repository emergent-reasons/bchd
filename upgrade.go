...
// oldBchdHomeDir returns the OS specific home directory bchd used prior to
// version 0.3.3.  This has since been replaced with bchutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldBchdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "bchd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".bchd")
	}
...
// location prior to bchd version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to bchd
...
// upgradeDataPaths moves the application data from its location prior to bchd
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		bchdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		if ohpEmpty {
			...
			bchdLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to bchd as new versions require it.
